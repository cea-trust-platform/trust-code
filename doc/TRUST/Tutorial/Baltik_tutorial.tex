...
\texttt{\$ git status -\,-u=no}\\
$\rightarrow$ But PAR\_upwind\_jdd1 is CORE.\\

\item Correct the datafile to run in parallel:\\
...
\begin{itemize}
\item For this, we analyze the test case:\\
...
Update share/Validation/Rapports\_automatiques/upwind/src/upwind.data\\
change "Scatter ../upwind/DOM.Zones dom" by "Scatter DOM.Zones dom"\\
\texttt{\$ cd \$project\_directory}\\
\texttt{\$ make check\_last\_pb\_optim}\\
...
\item You will have an error indicating a pure virtual function ("interpreter\_") should be
implemented.

\item Look at the "Interprete\_geometrique\_base" class thanks to F3 (Open Declaration)
from Eclipse and notice the "interpreter()" method.\\
\end{itemize}
\end{block}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\frametitle{Creation of a new cpp class}
\begin{itemize}
\item Add into the data file "Cx.data" the following 2 lines: the keyword "my\_first\_class" and "End" just after the line where the problem is discretized\\
NB: Without "End", you can reduce the number of time step to only 1.\\
...
\item Understand that "Interprete\_geometrique\_base::interpreter()" method is called first,
...
{\bf{Nom}} opt;\\
int dim;\\
Cerr <\textcompwordmark{}< " Reading parameters of A from a stream (cin or file) " <\textcompwordmark{}< finl;\\
...
\textcolor{orange}{param}.ajouter("dimension",\&dim,Param::REQUIRED); \textcolor{blue}{// Mandatory parameter}\\
\textcolor{blue}{// Read now the parameters from the stream is and produces an error if unknown}\\
\textcolor{blue}{// keyword is read or if brackets are not found at the beginning and the end:}\\
...
\begin{block}{}
\begin{itemize}
\item To call it in the data file, you have to use the following syntaxe:
\begin{center}
\fbox{ \begin{minipage}[c]{0.9\textwidth}
\begin{alltt}
A a\\
{\bf{Read}} a { dimension 3 option fast }
\end{alltt}
\end{minipage}}
\end{center}

\item Add into the "interpreter\_(Entree\&)" method the read of a domain and some parameters into brackets.

\item In the data file, the syntax will be:
\begin{center}
\fbox{ \begin{minipage}[c]{0.9\textwidth}
\begin{alltt}
{\bf{my\_first\_class}} dom \{ option 0 \} \textcolor{blue}{\# dom is the domain name \#}
\end{alltt}
\end{minipage}}
\end{center}

\item Use the following method to read the name of the domain:\\
Interprete\_geometrique\_base::associer\_domaine(Entree\&).

\item Look the HTML documentation. What is the task of this method?

\item To help you, have a look at a "Interprete\_geometrique\_base" sub-class, for
instance "Raffiner\_anisotrope" to see how the domain is read. 

\item The datafile syntax is:
\begin{center}
\fbox{ \begin{minipage}[c]{0.9\textwidth}
\begin{alltt}
{\bf{Raffiner\_anisotrope}} DomainName
\end{alltt}
\end{minipage}}
\end{center}
...
\begin{itemize}
\item Then use the \textbf{Param} object to read the keyword parameter option in the
data file. 
...
\item Add "\#include <Param.h>" into the cpp file and if help needed, have a look at
the "Interprete\_geometrique\_base" sub-class "Extruder". 
...
\fbox{ \begin{minipage}[c]{0.9\textwidth}
\begin{alltt}
Cerr <\textcompwordmark{}< "Option number " <\textcompwordmark{}< option\_number <\textcompwordmark{}< " has been read on the
domain named " <\textcompwordmark{}< ??? <\textcompwordmark{}< finl;
\end{alltt}
\end{minipage}}
\end{center}
\end{itemize}
...
\fbox{ \begin{minipage}[c]{0.9\textwidth}
\begin{alltt}
Cerr <\textcompwordmark{}< "The boundary named " <\textcompwordmark{}< ??? <\textcompwordmark{}< " has " <\textcompwordmark{}< ??? <\textcompwordmark{}< " faces." <\textcompwordmark{}< finl;
\end{alltt}
\end{minipage}}
\end{center}
\end{itemize}
...
\fbox{ \begin{minipage}[c]{0.9\textwidth}
\begin{alltt}
\textbf{my\_first\_class} dom \{ \textbf{option} 0 \textbf{problem} pb \}
\end{alltt}
\end{minipage}}
\end{center}
...
\fbox{ \begin{minipage}[c]{0.9\textwidth}
\begin{alltt}
Cerr <\textcompwordmark{}< control\_volumes.size() <\textcompwordmark{}< finl;
\end{alltt}
\end{minipage}}
\end{center}
