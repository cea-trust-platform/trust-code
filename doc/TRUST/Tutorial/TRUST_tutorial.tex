...
\title[TRUST Tutorial V1.7.7beta]{TRUST Tutorial V1.7.7beta} % The short title appears at the bottom of every slide, the full title is only on the title page
...
\begin{columns}[c] 
\column{.45\textwidth}
\tableofcontents[sections={1-9}, hideallsubsections]
\column{.5\textwidth} 
\tableofcontents[sections={10-16}, hideallsubsections]
\end{columns}
...
\textbf{mkdir -p \; $\sim$/Formation\_TRUST/yourname}\\
\textbf{cd \; $\sim$/Formation\_TRUST/yourname}
\vspace{0.2cm}
...
\item \label{save_restart} Edit your data file, with "Edit data" or "Edit with xdata" and modify \textbf{tinit\index{tinit}}, \textbf{tmax\index{tmax}} values in the object "mon\_schema".
...
\textbf{cd $\sim$/Formation\_TRUST/yourname/Obstacle} \\
\textbf{mkdir PARA1} \\
\textbf{cd PARA1} \\
...
\item We are going to change the discretization of the test case: triangulate the domains with the keyword \textbf{Trianguler\_H\index{Trianguler\_H}} (See the syntax in the Reference Guide).
\end{itemize}
...
\item \label{schema_impl} Use a full implicit scheme now (suppress \textbf{diffusion\_implicite\index{diffusion\_implicite}}), by substituting \textbf{Scheme\_Euler\_Explicit} by \textbf{Scheme\_Euler\_implicit\index{scheme\_euler\_implicit}} and adding the \textbf{Implicite} solver.

\item Have a look at the Reference Guide for the \textbf{gmres\index{gmres}} options and define accordingly to the advices given into the Reference Guide, a value for \textbf{facsec\index{facsec}, facsec\_max\index{facsec\_max}}.
\end{itemize}
...
{\small{
\textbf{mkdir -p $\sim$/Formation\_TRUST/yourname/PeriodicBox} \\
\textbf{cd $\sim$/Formation\_TRUST/yourname/PeriodicBox} \\
...
\textbf{mkdir -p $\sim$/Formation\_TRUST/yourname/salome/exo1} \\
\textbf{cd $\sim$/Formation\_TRUST/yourname/salome/exo1} \\
...
\begin{itemize}
\item Create a directory and run Salom\'e (we suppose it is installed):\\
\textbf{mkdir -p $\sim$/Formation\_TRUST/yourname/salome/exo2} \\
\textbf{cd $\sim$/Formation\_TRUST/yourname/salome/exo2} \\
...
\begin{itemize}
\item Create a directory and run Salom\'e (we suppose it is installed):\\
\textbf{mkdir -p $\sim$/Formation\_TRUST/yourname/salome/exo3} \\
\textbf{cd $\sim$/Formation\_TRUST/yourname/salome/exo3} \\
...
{\footnotesize{
\textbf{mkdir \, -p \, $\sim$/Formation\_TRUST/yourname/gmsh} \\
\textbf{cd \, $\sim$/Formation\_TRUST/yourname/gmsh} \\
\textbf{source /home/triou/env\_\textcolor{blue}{TrioCFD}\_X.Y.Z.sh }\\
\textbf{echo \$project\_directory} \\
...
\textbf{mkdir \, -p \, $\sim$/Formation\_TRUST/yourname/Xprepro/exo1} \\
\textbf{cd \, $\sim$/Formation\_TRUST/yourname/Xprepro/exo1} \\
\textbf{source /home/triou/env\_TRUST\_X.Y.Z.sh }\\
\textbf{Xprepro \&} 
\end{itemize}
...
\begin{itemize}
\item Note that the "maillagefefaut" file and your prepro file are saved in the directory $\sim$/Formation\_TRUST/yourname/Xprepro/model.
...
\textbf{mkdir \, -p \, $\sim$/Formation\_TRUST/yourname/Xprepro/exo2} \\
\textbf{cd \, $\sim$/Formation\_TRUST/yourname/Xprepro/exo2} \\
\textbf{source /home/triou/env\_TRUST\_X.Y.Z.sh }\\
\textbf{Xprepro \&} 
\end{itemize}
...
{\small{
\textbf{mkdir -p $\sim$/Formation\_TRUST/yourname/validation} \\
\textbf{cd  $\sim$/Formation\_TRUST/yourname/validation} \\
