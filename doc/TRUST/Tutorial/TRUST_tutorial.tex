...
\hspace{1.5cm} $\color{darkblue}\diamond$ {\footnotesize{ select "Controls $\rightarrow$ Line out", and define your profile with left button,}} \\
...
\hspace{1cm} $\color{darkblue}\circ$ {\small{You can create new fields (expression) with "Controls $\rightarrow$ Expressions \\
...
\item Now we want to change the data file to restart the calculation. So open it using "PLOT2D" tool: \\
\textbf{trust -evol\index{trust -evol} Obstacle \&}
\end{itemize}
...
\item Click on the "$\triangleright$" button of the Main Shape field and select the "Cylinder\_1" in the "Object browser" or in the visualization window.\\
