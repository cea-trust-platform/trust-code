...
\title[TRUST Tutorial V1.8.5beta]{TRUST Tutorial V1.8.5beta} % The short title appears at the bottom of every slide, the full title is only on the title page
...
\begin{itemize}
\item On new CEA Saclay PCs, TRUST versions are available with (e.g. X.Y.Z=1.8.4):\\
%\vspace{0.2cm}
\textbf{source  /home/trust\_trio-public/env\_TRUST-X.Y.Z.sh}
%\vspace{0.5cm}
\item On old CEA Saclay PCs, TRUST versions are available with (e.g. X.Y.Z=1.8.4):\\
%\vspace{0.2cm}
...
\begin{block}{}
Several editors (\textbf{vim}, \textbf{emacs}, \textbf{nedit}, \textbf{gedit}) can be configured to highlight TRUST keywords in the data files. 
...
\item Replace the keyword "\textbf{format lml}" with "\textbf{format lata\index{format lata}}" inside the block, just before the keyword \textbf{fields} in order to use the post-processing tool VisIt during and/or after the calculation.
\end{itemize}
...
\begin{itemize}
\item Login to the CEA cluster "orcus" and initialize the TRUST environment (for example, X.Y.Z = 1.8.4): \\
{\small{
...
{\small{
\begin{itemize}
\item On new CEA Saclay computers:\\
\textbf{source /home/trust\_trio-public/\textcolor{red}{full}\_env\_\textcolor{blue}{TrioCFD}-X.Y.Z.sh }

\item On old CEA Saclay computers:\\
...
\begin{itemize}
\item On new CEA Saclay computers:\\
\textbf{source /home/trust\_trio-public/env\_\textcolor{blue}{TrioCFD}-X.Y.Z.sh }
...
\item Load TRUST environment as described on page \pageref{init_trust} then run:\\
\textbf{Xprepro \&} 
\end{itemize}
...
\item Run the file:\\
 \textbf{trust channel}
\item Visualize your mesh with:\\
 \textbf{visit\index{visit} -o dom\_pb1.lata \& }
\end{itemize}
...
\begin{block}{Here is an example of Validation forms:}
