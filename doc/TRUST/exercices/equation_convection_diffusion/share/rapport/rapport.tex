...
\
 ...
 \item \underline{Notion de milieu} : Description du milieu fluide ou solide qu'on modélise. Le Milieu est défini dans le problème et est connu par les équations. Parmi les propriétés qui caractérisent le milieu, on a:
 \begin{itemize}
...
noter que cette instruction doit être exécutée dans le répertoire
...
Il est important de noter que lors de l'ajout de nouvelles sources dans 
...
\item \texttt{discretiser}: discrétise la vitesse de convection à partir de la vitesse de convection donné par l'utilisateur.
...
\begin{enumerate}
 \item le rajout de la référence \texttt{le\_constituant}. Pour ce faire, il est nécessaire d'inclure le fichier \texttt{TRUST\_Ref.h} et d'ajouter notre attribut en tant que référence  \texttt{REF(Constituant\_Avec\_Vitesse)}. Cette technique, non standard C++, est utilisée dans
