            ...
            while (!ok && !stop) {
                ...
                T->initTimeStep(dt);
                {
                    ...
                    if (dom1_group.containsMyRank()) {
                        ...
                        T->getOutputField("FLUX_SURFACIQUE_OUT_DOM1", field_flux); }
                    else {
                        T->getOutputField("TEMPERATURE_OUT_DOM2", field_temperature);
                        T->getInputFieldTemplate("FLUX_SURFACIQUE_IN_DOM2", field_flux);
                    }

                    if (init) {
                        dec_flux.setMethod(field_flux._type == 0 ? "P0" : "P1");
                        dec_temperature.setMethod(field_temperature._type == 0 ? "P0" : "P1");
                    }

                    // Attach the fields to the pipes
                    dec_flux.attachLocalField(&field_flux);
                    modifie_n_TrioDEC(dec_flux);
                    dec_temperature.attachLocalField( &field_temperature);
                    modifie_n_TrioDEC(dec_temperature);

                    if (init) {
                        dec_flux.synchronize();
                    ...
                    if (dom2_group.containsMyRank() )
                        T->setInputField("FLUX_SURFACIQUE_IN_DOM2", field_flux);
                    else
