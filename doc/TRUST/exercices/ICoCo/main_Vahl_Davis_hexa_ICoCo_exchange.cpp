...
int main(int argc,char **argv) {
  MPI_Init(&argc,&argv);
  {
    ...
    data_file="Vahl_Davis_hexa_ICoCo_exchange"; // name of the first dom.data file // + ICoCo couplage
    // Redirection des sorties couplage dans dom.out et dom.err
    if (!freopen("Vahl_Davis_hexa_ICoCo_exchange.out","w",stdout)) abort();         // + ICoCo couplage
    if (!freopen("Vahl_Davis_hexa_ICoCo_exchange.err","w",stderr)) abort();         // + ICoCo couplage
    T = getProblem();
