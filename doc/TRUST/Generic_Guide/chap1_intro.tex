...
\item in that document, words in \textbf{bold} are \trust keywords, you can enlight them in your file editor with command line (details in section \ref{Run}):\\
\texttt{> trust -config nedit|vim|emacs}
\item braces "\{ \}" are elements that \trust reads and interpretes, so don't forget them and \underline{put space before and after them},
