...
\begin{array}{c}
\nabla \cdot \vec u =0 \\
\displaystyle{\frac{\partial \vec u }{\partial t} + \textcolor{red}{\vec u \nabla \vec u} = - \nabla P^* + \textcolor{blue}{\nabla \cdot (\nu \nabla \vec u)} } \\
\displaystyle{\frac{\partial T}{\partial t} + \vec u \nabla T = \nabla \cdot (\alpha \nabla T) }
\end{array}
\right.
$$

or for quasi-compressible fluid:

$$
\left\{
\begin{array}{c}
\displaystyle{\frac{\partial \rho }{\partial t} + \nabla \cdot (\rho \vec u) =0 }\\
\displaystyle{ \frac{\partial}{\partial t} (\rho u_i) + \textcolor{red}{div (\rho u u_i)} - \textcolor{blue}{\sum_{j=1}^N  \frac{\partial}{\partial x_j} \left[\mu \left( \frac{\partial u_i}{\partial x_j} + \frac{\partial u_j}{\partial x_i} \right) \right] } + \frac{\partial P^*_1}{\partial x_i} = -\rho g_i} \\
\displaystyle{ \rho C_p \frac{dT}{dt} - \sum_{j=1}^N \frac{\partial}{\partial x_j} \left( \lambda \frac{\partial T}{\partial x_j} \right) = Q + \frac{dP_0}{dt} }
\end{array}
\right.
$$

where: $\displaystyle{P^*=\frac{P}{\rho} + g z}$, $P_0=\rho R T$, $\displaystyle{P_1^*=P_1 + \frac{2}{3} \mu div (\vec u )}$, $Q$ is a heat source term, and:
...
\cline{1-4} 
Thermo & hydraulique & Concentration & Turbulent & \multicolumn{1}{c}{}\tabularnewline
\hline 
\hline 
            & \textbf{Pb\_hydraulique}  &   
            &                           & \href[page=DOCLINK_HYDRAULIC PROBLEM]{User_Manual_TRUST.pdf}{doc} \tabularnewline 
\hline
            & \textbf{Pb\_hydraulique}  & \textbf{\textcolor{darkblue}{\_Concentration}}
            &                           & \href[page=DOCLINK_HYDRAULIC PROBLEM WITH CONCENTRATION]{User_Manual_TRUST.pdf}{doc} \tabularnewline
\hline
            & \textbf{Pb\_hydraulique}  &   
            & \textbf{\textcolor{Greeen}{\_Turbulent}}      & \href[page=DOCLINK_TURBULENT HYDRAULIC PROBLEM]{User_Manual_TRUST.pdf}{doc} \tabularnewline
\hline
            & \textbf{Pb\_hydraulique}  & \textbf{\textcolor{darkblue}{\_Concentration}}
            & \textbf{\textcolor{Greeen}{\_Turbulent}}      & \href[page=DOCLINK_TURBULENT HYDRAULIC PROBLEM WITH CONCENTRATION]{User_Manual_TRUST.pdf}{doc} \tabularnewline
\hline
\textbf{Pb\_\textcolor{magenta}{Thermo}}     & \textbf{hydraulique}  &   
                        &                       & \href[page=DOCLINK_THERMOYDRAULIC PROBLEM]{User_Manual_TRUST.pdf}{doc} \tabularnewline
\hline
\textbf{Pb\_\textcolor{magenta}{Thermo}}     & \textbf{hydraulique}  & \textbf{\textcolor{darkblue}{\_Concentration}}
                        &                       & \href[page=DOCLINK_THERMOHYDRAULIC PROBLEM WITH CONCENTRATION]{User_Manual_TRUST.pdf}{doc} \tabularnewline
\hline
\textbf{Pb\_\textcolor{magenta}{Thermo}}     & \textbf{hydraulique}  &   
                        & \textbf{\textcolor{Greeen}{\_Turbulent}}  & \href[page=DOCLINK_TURBULENT THERMOHYDRAULIC PROBLEM]{User_Manual_TRUST.pdf}{doc} \tabularnewline
\hline
\textbf{Pb\_\textcolor{magenta}{Thermo}}     & \textbf{hydraulique}  & \textbf{\textcolor{darkblue}{\_Concentration}}
                        & \textbf{\textcolor{Greeen}{\_Turbulent}}  & \href[page=DOCLINK_THERMOHYDRAULIC TURBULENT PROBLEM WITH CONCENTRATION]{User_Manual_TRUST.pdf}{doc} \tabularnewline \hline
\end{longtable}
...
\begin{alltt}
{\bf{Pb\_Thermohydraulique\textcolor{Greeen}{\_Turbulent}\_QC\textcolor{mauve}{\_fraction\_massique}} } \textit{my\_problem}
...
For more information on thermohydraulique QC problem, go \href[page=DOCLINK_PROBLEM FOR NAVIER STOKES EQUATIONS UNDER A SMALL MACH NUMBER APPROXIMATION]{User_Manual_TRUST.pdf}{here} and for thermohydraulique turbulent QC problem, go \href[page=DOCLINK_TURBULENT THERMOHYDRAULICAL PROBLEM UNDER SMALL MACH NUMBER]{User_Manual_TRUST.pdf}{there}.
...
For this kind of problem, \trust solves the heat equation. In your data file, you will have:
...
For more information, see the \href[page=DOCLINK_CONDUCTION PROBLEM]{User_Manual_TRUST.pdf}{User's Manual}.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Pressure solvers}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Then you may indicate the choice of pressure solver (cf \href[page=DOCLINK_PRESSURE SOLVERS]{User_Manual_TRUST.pdf}{User's Manual}) using the following syntaxe:
...
\begin{itemize}
\item \href[page=DOCLINK_PRECONDITIONED CONJUGATED GRADIENT]{User_Manual_TRUST.pdf}{\textbf{GCP}},
\item \href[page=DOCLINK_SOLVERS FROM PETSC API]{User_Manual_TRUST.pdf}{\textbf{Petsc} \textit{Petsc\_solver\_name}},
\item \href[page=DOCLINK_CHOLESKY DIRECT METHOD]{User_Manual_TRUST.pdf}{\textbf{Cholesky}},
\item \textbf{Gmres} or \textbf{Gen} or \textbf{Optimal} (cf to the \href[page=DOCLINK_OTHER SOLVERS]{User_Manual_TRUST.pdf}{User's Manual}).
\end{itemize}
...
\section{Convection}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
There is no default convectif scheme so you must choose one (cf \href[page=DOCLINK_CONVECTIVE SCHEMES]{User_Manual_TRUST.pdf}{User's Manual}):
...
You can use the following convective scheme, following the recommendations of the user training session (cf \href[page=30]{TRUST_and_TrioCFD_presentation.pdf}{User Training Session} or his \href[page=135]{TRUST_and_TrioCFD_presentation.pdf}{Recommendations}) following your discretization type:
\begin{itemize}
\item \textbf{Amont}
\item \textbf{Muscl}
\item \textbf{EF\_stab}
\item for more, see the \href[page=DOCLINK_CONVECTIVE SCHEMES]{User_Manual_TRUST.pdf}{User's Manual}.
\end{itemize}
...
you can choose your scheme with the help of the \href[page=DOCLINK_DIFFUSIVE SCHEME]{User_Manual_TRUST.pdf}{User's Manual}.\\
...
To see the syntaxe of each available initial condition: cf \href[page=DOCLINK_INITIAL CONDITIONS]{User_Manual_TRUST.pdf}{User's Manual}.
...
\item for more, see the \href[page=DOCLINK_INITIAL CONDITIONS]{User_Manual_TRUST.pdf}{User's Manual}.
\end{itemize}
...
\item refer to \href[page=DOCLINK_OBJECT FIELD DEFINITION]{User_Manual_TRUST.pdf}{User's Manual}.
\end{itemize}
...
You can find help for boundary conditions in the \href[page=DOCLINK_BOUNDARY CONDITIONS]{User_Manual_TRUST.pdf}{User's Manual}.
...
\item for more, see the \href[page=DOCLINK_BOUNDARY CONDITIONS]{User_Manual_TRUST.pdf}{User's Manual}.
\end{itemize}
}}

To choose your "boundary\_field\_type" parameters, refer to \href[page=DOCLINK_STATIONARY BOUNDARY FIELDS]{User_Manual_TRUST.pdf}{User's Manual}.
...
\begin{itemize}
\item \href[page=DOCLINK_THE MIXING LENGTH MODEL]{User_Manual_TRUST.pdf}{\textbf{Longueur\_Melange}}: RANS model based on mixing length modelling,
\item \href[page=DOCLINK_SUB-GRID SCALE MODELS]{User_Manual_TRUST.pdf}{\textbf{Sous\_maille}}: LES model which uses a structure sub-grid function model,
%\item \textbf{Sous\_maille\_Smago}
%\item \textbf{Sous\_maille\_wale}
\item \href[page=DOCLINK_THE K-EPSILON MODEL]{User_Manual_TRUST.pdf}{\textbf{K\_epsilon}}: for RANS turbulence model (k-$\varepsilon$),
%\item \textbf{K\_epsilon\_2\_Couches}
\item for more, see the \href[page=DOCLINK_TURBULENCE MODELS]{User_Manual_TRUST.pdf}{User's Manual}.
\end{itemize}
...
\item \href[page=DOCLINK_HYDRAULIC SOURCE TERMS]{User_Manual_TRUST.pdf}{documentation for hydraulic source terms},
\item \href[page=DOCLINK_SCALAR SOURCE TERMS]{User_Manual_TRUST.pdf}{documentation for scalar source terms}.
\end{itemize}
...
But you can also create a list of postprocessing with \textbf{Post\_processings} keyword (named with Post\_name1, Post\_name2, etc...), in order to print results to several formats or with different time periods, or into different results files:
...
We present here basic methods to post-process, you can find more details in \href[page=DOCLINK_PROBLEM POST-PROCESSING]{User_Manual_TRUST.pdf}{User's Manual}.\\
...
For more informations, you can see the \href[page=DOCLINK_POST-PROCESSING FIELD NAMES]{User_Manual_TRUST.pdf}{User's Manual}.
\vspace{1cm}

\item \textcolor{darkblue}{\textbf{Creating new fields}}

The \textbf{Definition\_champs} keyword is used to create new or more complex field for advanced postprocessing.
...
%\item Predefini
%\item Tparoi\_VEF
\item for details and other keywords, see the \href[page=DOCLINK_ADVANCED FIELD POST-PROCESSING]{User_Manual_TRUST.pdf}{User's Manual}.
\end{itemize}
...
You can find other examples in the \href[page=66]{TRUST_and_TrioCFD_presentation.pdf}{TRUST \& TrioCFD presentation} or in the \href[page=233]{User_Manual_TRUST.pdf}{User's Manual}.
...
Here is an example of 2D \textbf{Probes} block:
    ...
    \end{minipage}}
    \end{center}
where the use of \textit{"loc"} option allow to specify the wanted localisation of the probes. The available values are \textbf{"grav"} for gravity center of the element, \textbf{"nodes"} for faces and \textbf{"som"} for verteces.

For complete syntax, see the User's Manual \href[page=222]{User_Manual_TRUST.pdf}{here}, and for all options see the \href[page=DOCLINK_POST-PROCESSING BY PROBE]{User_Manual_TRUST.pdf}{User's Manual}.
...
\Note that \underline{when you don't specify the localisation of the probe}, the default value is \textbf{"som"} for values at the verteces. So fields are post-processed at the verteces of the mesh.\\
...
For complete syntax, see the \href[page=222]{User_Manual_TRUST.pdf}{User's Manual here}, and for all options see the \href[page=DOCLINK_GENERAL FIELD POST-PROCESSING]{User_Manual_TRUST.pdf}{User's Manual there}.
...
For complete syntax, see the User's Manual \href[page=222]{User_Manual_TRUST.pdf}{here}, and for all options see the \href[page=DOCLINK_FIELD GENERAL POST-PROCESSING FOR STATISTICS]{User_Manual_TRUST.pdf}{User's Manual}.
\end{itemize}
