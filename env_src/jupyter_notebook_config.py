...
#  Choices: any of ['sha1', 'sha3_384', 'sha384', 'sha3_224', 'sha512', 'sha3_256', 'blake2b', 'sha3_512', 'sha224', 'blake2s', 'sha256', 'md5']
#  Default: 'sha256'
...
def scrub_output_pre_save(model, **kwargs):
    """ Scrub output before saving notebooks by default, except if 
              run.saveFormOutput() 
        has been invoked.
    """
    discardOutput = True
    ...
    # First loop to detect whether run.saveFormOutput() has been invoked
    for cell in model['content']['cells']:
        if cell['cell_type'] != 'code': continue
        ls = cell['source'].split('\n')
        for l in ls:
            l = l.strip()
            if "run.saveFormOutput()" in l and not l.startswith("#"): 
                discardOutput = False
        # Always do this, this is just annoying and useless (?)
        cell['execution_count'] = None
    
    if discardOutput:
        print("        (outputs are being discarded ... use 'run.saveFormOutput()' in your notebook to prevent this.)")
        for cell in model['content']['cells']:
            if cell['cell_type'] != 'code': continue
            cell['outputs'] = []
