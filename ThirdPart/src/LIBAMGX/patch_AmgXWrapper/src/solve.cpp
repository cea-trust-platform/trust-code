...
/* \implements AmgXSolver::solve */
PetscErrorCode AmgXSolver::solve(PetscScalar* p, const PetscScalar* b, const int nRows, const double tolerance)
{
    PetscFunctionBeginUser;
    ...
    if (gpuWorld != MPI_COMM_NULL)
    {
      ...
      // We compute the residual before solve to avoid a crash if already converged:
      double t_norm = 0;
