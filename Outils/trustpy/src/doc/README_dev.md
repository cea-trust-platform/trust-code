...
This is the developper's documentation of `trustpy`. If you are looking at how to use it you should turn to the root README file which is more user-oriented.
...
**NB:** note that this format existed before `trustpy` was put into place. In the future, this file/format might be converted to a more standard format (JSON?).
...
NB: mode -2 and -3 are not used in `trustpy`.
...
When `attr` is followed by `ref type_name` the class must be associated with a class of type `type_name` (keyword `associer` in the dataset). This is not checked currently in `trustpy` logic.
...
- `ref_toto` expects a name refering to an already created object in the dataset. In `trustpy` this is just read as a chaine
...
The `trustpy` package is made of several Python modules:
...
- `trad2_code_gen.py` which generates the Python module (usually named something like `trust_packagespy_generated_xxx.py` or `trustpy_gen.py`) 
...
- `trust_parser.py` which contains the parsing logic (actually it is just the tokens extractions for what is done in `trustpy` - this file was taken from a former process)
...
In `trustpy` forward declarations are mapped to the specific class `Declaration_Tru`. 
