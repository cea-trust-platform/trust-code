...
class TRUSTSuite(object):
    ...
    def copySrc(self):
        ...
        subprocess.run("mkdir -p build && cp -ar src/* %s" % self.buildDir_, shell=True)  # Note the '*' !!
    ...
    def runCases(self, verbose=False, preventConcurrent=False):
        ...
        runParallel = not preventConcurrent and \
                        ((opt is None or "-parallel_sjob" in opt.split(' ')) and self.detectSserver())
