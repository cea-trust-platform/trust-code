...
class Show(object):
    ...
    """
    def __init__(self,fichier="",field="",name="",nX=1,nY=1,mesh="dom",
                 plotmesh=True,time=-1,empty=False,size=10,title="",max=None,min=None,active=True,
                 visitLog=False,verbose=0,show=True):
        """
        Constructeur
    ...
    def executeVisitCmds(self):
        ...
        outp = subprocess.run("visit -nowin -cli -s %s 1>> visit.log 2>&1" % visitTmpFile_(justFile=True),
                              shell=True)
        os.chdir(origin)
...
#             os.system("visit -nowin -cli -s  tmp.py") # Faire sa independante de cette methode
    ...
    def add(self,fichier,field,name,xIndice=0,yIndice=0,time=-1,mesh="dom",title="",plotmesh=True,max=None,min=None):
        """
    ...
    def plot(self, show=True):
        ...
        if self.show: self.insert()
        if self.show and show: plt.show()
