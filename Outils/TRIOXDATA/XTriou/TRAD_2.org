...
ice piso ice -1 Implicit Continuous-fluid Eulerian solver which is useful for a multiphase problem. Robust pressure reduction resolution.
  attr criteres_convergence criteres_convergence criteres_convergence 1 Set the convergence thresholds for each unknown (i.e: alpha, temperature, velocity and pressure). The default values are respectively 0.01, 0.1, 0.01 and 100 
...
frontiere_ouverte neumann frontiere_ouverte -1 Boundary outlet condition on the boundary called bord (edge) (diffusion flux zero). This condition must be associated with a boundary outlet hydraulic condition.
  attr var_name chaine(into=["T_ext","C_ext","Y_ext","K_Eps_ext","Fluctu_Temperature_ext","Flux_Chaleur_Turb_ext","V2_ext","a_ext"]) var_name 0 Field name.
  attr ch front_field_base ch 0 Boundary field type.
...
saturation_base fluide_base saturation_base 1 Basic class for phase change management (used in pb_multiphase)
...
frottement_interfacial source_base frottement_interfacial 0 Source term which corresponds to the phases friction at the interface
  attr model chaine(into=["bulles","wallis","sonnenburg"]) model 1 Correlation for friction in bubbly flows if bulles, Correlation for drift flux of Sonnenburg if sonnenburg or Correlation for friction in annular flows if wallis
  attr bloc_bulles bloc_b bloc_bulles 1 not set
bloc_b interprete nul 1 not set
  attr rayon_bulle floattant rayon_bulle 1 Radius of the bubbles (useful for the correlation and it is required)
  attr coeff_derive floattant coeff_derive 1 Drift coefficient (useful for the correlation and it is required)
  
typer_lire interprete typer_lire 0 TRUST interpretor which declares and reads the object
  attr object_name chaine object_name 0 Name of the object that will be read
  attr object_type fluide_base object_type 1 Type of the instanced object. TODO: in XDATA, we use it for fluids only
  attr proprietes bloc_proprietes propriete 1 Read the properties of the object
bloc_proprietes interprete nul 1 definition of object properties. TODO: in XDATA we use it for fluids only
...
phases interprete phases 0 Declare the phases that will be considered
  attr problem_name ref_Pb_base problem_name 0 Name of problem.
  attr phases_def bloc_phases phases_def 0 bloc to define phases
bloc_phases interprete nul 1 not_set
  attr liquide bloc_lecture liquide 1 definition of the liquid phase
  attr gaz bloc_lecture gaz 1 definition of the gazeous phase
...
Fluide_sodium_gaz fluide_reel_base Fluide_sodium_gaz -1 Class for Fluide_sodium_liquide
  attr P_ref floattant P_ref 1 Use to set the pressure value in the closure law. If not specified, the value of the pressure unknown will be used
  attr T_ref floattant T_ref 1 Use to set the temperature value in the closure law. If not specified, the value of the temperature unknown will be used
StiffenedGas fluide_reel_base StiffenedGas -1 Class for Stiffened Gas
