...
gen solveur_sys_base gen -1 not_set
  ...
  attr precond chaine precond 0 The only preconditionner that we can specify is ilu. Its syntax is : precond ilu { [type int] [filling int] } where type is an integer with values 0|1|2|3 for null|left|right|left-and-right preconditionning (default value = 2) and filling is an integer (default value = 1).
  attr seuil floattant seuil 1 Value of the final residue. The solver ceases iterations when the Euclidean residue standard ||Ax-B|| is less than this value. defautl value 1e-12.
