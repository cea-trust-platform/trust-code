...
c     routine named distdot2. The prototype of distdot2 is
c
c     real*8 function distdot2(n,x,ix,y,iy)
c     integer n, ix, iy
c     real*8 x(1+(n-1)*ix), y(1+(n-1)*iy)
c
c     This interface of distdot2 is exactly the same as that of
...
c     processor. (d) distdot2 should be replaced by a distributed
...
c     convergence tests.
c-----------------------------------------------------------------------
      subroutine cgv2(n, rhs, sol, ipar, fpar, w)
      implicit none
      integer n, ipar(16)
...
c
      real*8 distdot2
      logical stopbis, brkdnv2
      external distdot2, stopbis, brkdnv2, bisinitv2
c
c     local variables
c
      integer i
...
c     initialization
c
      call bisinitv2(ipar,fpar,5*n,1,lp,rp,w)
...
c
      fpar(7) = distdot2(n,w,1,w,1)
      fpar(11) = fpar(11) + 2 * n
      fpar(3) = sqrt(fpar(7))
      fpar(5) = fpar(3)
      if (abs(ipar(3)).eq.2) then
         fpar(4) = fpar(1) * sqrt(distdot2(n,rhs,1,rhs,1)) + fpar(2)
         fpar(11) = fpar(11) + 2 * n
...
c
 60   ipar(7) = ipar(7) + 1
      alpha = distdot2(n,w(1,2),1,w(1,3),1)
      fpar(11) = fpar(11) + 2*n
      if (brkdnv2(alpha,ipar)) goto 900
      alpha = fpar(7) / alpha
      do i = 1, n
...
c
 ...
 80   if (rp) then
         call tidycgv2(n,ipar,fpar,sol,w(1,4))
      else
         call tidycgv2(n,ipar,fpar,sol,w(1,5))
      endif
c
      return
      end
c-----end-of-cg
c-----------------------------------------------------------------------
      subroutine cgnrv2(n,rhs,sol,ipar,fpar,wk)
      implicit none
      integer n, ipar(16)
...
c     initialization
c
      call bisinitv2(ipar,fpar,5*n,1,lp,rp,wk)
      if (ipar(1).lt.0) return
c
c     request for matrix vector multiplication A*x in the initialization
c
...
c
 60   ipar(7) = ipar(7) + 1
      zzm1 = zz
      zz = distdot2(n,wk(1,3),1,wk(1,3),1)
      fpar(11) = fpar(11) + 2 * n
      if (brkdnv2(zz,ipar)) goto 900
      if (ipar(7).gt.3) then
         alpha = zz / zzm1
         do i = 1, n
...
c
 90   ipar(7) = ipar(7) + 1
      alpha = distdot2(n,wk(1,3),1,wk(1,3),1)
      fpar(11) = fpar(11) + 2 * n
      if (brkdnv2(alpha,ipar)) goto 900
      alpha = zz / alpha
      do i = 1, n
...
c
 ...
 110  if (rp) then
         call tidycgv2(n,ipar,fpar,sol,wk(1,4))
      else
         call tidycgv2(n,ipar,fpar,sol,wk(1,5))
      endif
      return
      end
c-----end-of-cgnrv2
c-----------------------------------------------------------------------
      subroutine bcgv2(n,rhs,sol,ipar,fpar,w)
      implicit none
      integer n, ipar(16)
...
c
      real*8 distdot2
      logical stopbis,brkdnv2
      external distdot2, stopbis, brkdnv2
c
      real*8 one
      parameter(one=1.0D0)
c
...
c
      call bisinitv2(ipar,fpar,7*n,1,lp,rp,w)
      if (ipar(1).lt.0) return
c
...
c
 60   ipar(7) = ipar(7) + 1
      alpha = distdot2(n,w(1,4),1,w(1,5),1)
      fpar(11) = fpar(11) + 2 * n
      if (brkdnv2(alpha,ipar)) goto 900
      alpha = fpar(7) / alpha
      do i = 1, n
...
c
 100  ipar(7) = ipar(7) + 1
      ...
      enddo
      fpar(8) = fpar(7)
      fpar(7) = distdot2(n,w,1,w(1,2),1)
      fpar(11) = fpar(11) + 4 * n
      if (brkdnv2(fpar(7), ipar)) return
      alpha = fpar(7) / fpar(8)
      do i = 1, n
...
c
 ...
 110  if (rp) then
         call tidycgv2(n,ipar,fpar,sol,w(1,6))
      else
         call tidycgv2(n,ipar,fpar,sol,w(1,7))
      endif
      return
c-----end-of-bcg
      end
c-----------------------------------------------------------------------
      subroutine bcgstabv2(n, rhs, sol, ipar, fpar, w)
      implicit none
      integer n, ipar(16)
...
c
      call bisinitv2(ipar,fpar,8*n,1,lp,rp,w)
      if (ipar(1).lt.0) return
c
...
c
c     step (2)
      alpha = distdot2(n,w(1,1),1,w(1,5),1)
      fpar(11) = fpar(11) + 2 * n
      if (brkdnv2(alpha, ipar)) goto 900
      alpha = fpar(7) / alpha
      fpar(8) = alpha
...
c
c     step (5)
      omega = distdot2(n,w(1,4),1,w(1,4),1)
      fpar(11) = fpar(11) + n + n
      if (brkdnv2(omega,ipar)) goto 900
      omega = distdot2(n,w(1,4),1,w(1,3),1) / omega
      fpar(11) = fpar(11) + n + n
      if (brkdnv2(omega,ipar)) goto 900
      fpar(9) = omega
      alpha = fpar(8)
...
c     step (8): computing new p and rho
      rho = fpar(7)
      fpar(7) = distdot2(n,w(1,2),1,w(1,1),1)
      omega = fpar(9)
      ...
      if (brkdnv2(fpar(7),ipar)) goto 900
c
...
c
 ...
 110  if (rp) then
         ...
         call tidycgv2(n,ipar,fpar,sol,w(1,8))
      endif
c
      return
c-----end-of-bcgstab
...
c-----------------------------------------------------------------------
      subroutine tfqmrv2(n, rhs, sol, ipar, fpar, w)
      implicit none
      integer n, ipar(16)
      real*8 rhs(n), sol(n), fpar(16), w(n,*)
c-----------------------------------------------------------------------
...
c     initializations
c
      call bisinitv2(ipar,fpar,11*n,2,lp,rp,w)
      if (ipar(1).lt.0) return
      ipar(1) = 1
      ipar(8) = 1
      ipar(9) = 1 + 6*n
...
c
      fpar(5) = sqrt(distdot2(n,w,1,w,1))
      fpar(3) = fpar(5)
      tao = fpar(5)
      fpar(11) = fpar(11) + n + n
      if (abs(ipar(3)).eq.2) then
...
c
 30   sigma = rho
      rho = distdot2(n,w(1,2),1,w(1,3),1)
      fpar(11) = fpar(11) + n + n
      if (brkdnv2(rho,ipar)) goto 900
      if (ipar(7).eq.1) then
         alpha = zero
      else
         alpha = rho / sigma
...
c
 ...
 60   ipar(7) = ipar(7) + 1
      ...
      enddo
      sigma = distdot2(n,w(1,2),1,w(1,8),1)
      fpar(11) = fpar(11) + 6 * n
      if (brkdnv2(sigma,ipar)) goto 900
      alpha = rho / sigma
      do i = 1, n
...
c     update I
c
      theta = distdot2(n,w(1,3),1,w(1,3),1) / (tao*tao)
      sigma = one / (one + theta)
      tao = tao * sqrt(sigma * theta)
      fpar(11) = fpar(11) + 4*n + 6
      if (brkdnv2(tao,ipar)) goto 900
      eta = sigma * alpha
      sigma = te / alpha
      te = theta * eta
      do i = 1, n
      ...
      if (ipar(7).eq.1) then
         if (ipar(3).eq.-1) then
            fpar(3) = eta * sqrt(distdot2(n,w(1,9),1,w(1,9),1))
            fpar(4) = fpar(1)*fpar(3) + fpar(2)
...
c     this is an approximation
      ...
      if (ipar(3).eq.999) then
         ...
         fpar(6) = eta * sqrt(distdot2(n,w(1,9),1,w(1,9),1))
...
c
 ...
 110  if (rp) then
         call tidycgv2(n,ipar,fpar,sol,w(1,10))
      else
         call tidycgv2(n,ipar,fpar,sol,w(1,11))
      endif
c
      return
      end
c-----end-of-tfqmrv2
c-----------------------------------------------------------------------
      subroutine fomv2(n, rhs, sol, ipar, fpar, w)
      implicit none
      integer n, ipar(16)
...
c
      ...
      vrn = vs + m
      i = vrn + m + 1
      call bisinitv2(ipar,fpar,i,1,lp,rp,w)
      if (ipar(1).lt.0) return
c
c     request for matrix vector multiplication A*x in the initialization
c
 ...
 10   ipar(7) = ipar(7) + 1
      ...
      fpar(11) = fpar(11) + n
c
 20   alpha = sqrt(distdot2(n,w,1,w,1))
...
c
 50   ipar(7) = ipar(7) + 1
      ptr = k * (k - 1) / 2 + hes
      p2 = ipar(9)
      call mgsrov2(.false.,n,n,k+1,k+1,fpar(11),w,w(ptr+1),
     $     ipar(12))
...
c-----end-of-fomv2-------------------------------------------------------- 
c-----------------------------------------------------------------------
      subroutine gmresv2(n, rhs, sol, ipar, fpar, w)
      implicit none
      integer n, ipar(16)
      real*8 rhs(n), sol(n), fpar(16), w(*)
c-----------------------------------------------------------------------
...
c-----end-of-gmresv2
c-----------------------------------------------------------------------
      subroutine dqgmresv2v2(n, rhs, sol, ipar, fpar, w)
      implicit none
      integer n, ipar(16)
      real*8 rhs(n), sol(n), fpar(16), w(*)
c-----------------------------------------------------------------------
...
c
      ...
      real*8 alpha,beta,psi,c,s,distdot2
      logical lp,rp,full
      external distdot2,bisinitv2
      save
c
c     where to go
c
...
c
 10   ipar(7) = ipar(7) + 1
      ...
      fpar(11) = fpar(11) + n
c
 20   alpha = sqrt(distdot2(n, w(iv+1), 1, w(iv+1), 1))
      fpar(11) = fpar(11) + (n + n)
      if (abs(ipar(3)).eq.2) then
         fpar(4) = fpar(1) * sqrt(distdot2(n,rhs,1,rhs,1)) + fpar(2)
         fpar(11) = fpar(11) + 2*n
...
c
 60   continue
      call mgsrov2(full,n,n,lb,jp1,fpar(11),w(iv+1),w(ihm+1),
     $     ipar(12))
...
c
      ...
      do while (i.ne.j)
         ...
         i = k
      enddo
      call givensv2(w(ihm+j), beta, c, s)
      if (full) then
...
c
      ...
      if (ipar(3).lt.0) then
         ...
         if (ipar(7).eq.2 .and. ipar(3).eq.-1) then
            fpar(3) = alpha*sqrt(distdot2(n, w(ptrw), 1, w(ptrw), 1))
            fpar(4) = fpar(1) * fpar(3) + fpar(2)
            fpar(6) = fpar(3)
         else
            fpar(6) = alpha*sqrt(distdot2(n, w(ptrw), 1, w(ptrw), 1))
         endif
         fpar(11) = fpar(11) + 2 * n
      else
         fpar(6) = fpar(5)
...
c-----end-of-dqgmresv2v2
c-----------------------------------------------------------------------
      subroutine fgmresv2v2(n, rhs, sol, ipar, fpar, w)
      implicit none
      integer n, ipar(16)
      real*8 rhs(n), sol(n), fpar(16), w(*)
c-----------------------------------------------------------------------
...
c-----end-of-fgmresv2v2
c-----------------------------------------------------------------------
      subroutine dbcgv2 (n,rhs,sol,ipar,fpar,w)
      implicit none
      integer n,ipar(16)
      real*8 rhs(n), sol(n), fpar(16), w(n,*)
c-----------------------------------------------------------------------
...
c     communication
c
c 2) impluv2, uppdirv2, distdot2 (blas)
c-----------------------------------------------------------------------
...
c
      ...
      parameter(one=1.0D0,zero=0.0D0)
c
      real*8 t,sqrt,distdot2,ss,res,beta,ss1,delta,x,zeta,umm
...
c
      call bisinitv2(ipar,fpar,11*n,1,lp,rp,w)
      if (ipar(1).lt.0) return
      perm(1) = .false.
      perm(2) = .false.
      perm(3) = .false.
...
c
 ...
 110  ipar(7) = ipar(7) + 1
      ...
      fpar(11) = fpar(11) + n
c
 120  fpar(3) = sqrt(distdot2(n,w(1,3),1,w(1,3),1))
      fpar(11) = fpar(11) + n + n
      fpar(5) = fpar(3)
      fpar(7) = fpar(3)
      zeta = fpar(3)
...
c
      if (k .ne. i2) then
         ...
         ss = distdot2(n,w(1,1),1,w(1,4+k),1)
         fpar(11) = fpar(11) + 2*n
         ss1= ss
         ju = ju + 1
         u(ju) = ss
...
c-----------------------------------------------------------------------
      np = min0(i,lb)
      full = (i .ge. lb)
      call impluv2(np, umm, beta, ypiv, u, perm, full)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      do 33 k=1,n
         w(k,1) = w(k,ip2+2)
 33   continue
      call uppdirv2(n, w(1,7), np, lb, indp, w, u, usav, fpar(11))
c-----------------------------------------------------------------------
      if (i .eq. 1) goto 34
      j = np - 1
      if (full) j = j-1
...
c-----------------------------------------------------------------------
 36   if (ipar(3).eq.999) then
      ...
      fpar(5) = res * sqrt(distdot2(n, w(1,i2+2), 1, w(1,i2+2), 1))
      fpar(11) = fpar(11) + 2 * n
      if (ipar(3).lt.0) then
         fpar(6) = x * sqrt(distdot2(n,w,1,w,1))
         fpar(11) = fpar(11) + 2 * n
         if (ipar(7).le.3) then
            fpar(3) = fpar(6)
            if (ipar(3).eq.-1) then
...
c-----end-of-dbcgv2-------------------------------------------------------
c-----------------------------------------------------------------------
      subroutine impluv2(np,umm,beta,ypiv,u,permut,full)
...
c-----------------------------------------------------------------------
      subroutine uppdirv2(n,p,np,lbp,indp,y,u,usav,flops)
...
c-----------------------------------------------------------------------
      ...
      real*8 fpar(16), r(n), delx(n), sx, distdot2
      external distdot2
c-----------------------------------------------------------------------
...
c     computes errors
c
      fpar(5) = sqrt(distdot2(n,r,1,r,1))
      fpar(11) = fpar(11) + 2 * n
      if (ipar(3).lt.0) then
c
c     compute the change in the solution vector
c
         fpar(6) = sx * sqrt(distdot2(n,delx,1,delx,1))
         fpar(11) = fpar(11) + 2 * n
...
c-----end-of-stopbis
c-----------------------------------------------------------------------
      subroutine tidycgv2(n,ipar,fpar,sol,delx)
      implicit none
      integer i,n,ipar(16)
...
c-----end-of-tidycgv2
c-----------------------------------------------------------------------
      logical function brkdnv2(alpha, ipar)
      implicit none
      integer ipar(16)
...
c-----end-of-brkdnv2
c-----------------------------------------------------------------------
      subroutine bisinitv2(ipar,fpar,wksize,dsc,lp,rp,wk)
      implicit none
...
c-----------------------------------------------------------------------
      subroutine mgsrov2(full,lda,n,m,ind,ops,vec,hh,ierr)
      implicit none
      logical full
...
c               -3: input vector is a linear combination of others
c
c     External routines used: real*8 distdot2
c-----------------------------------------------------------------------
      integer i,k
      real*8  nrm0, nrm1, fct, thr, distdot2, zero, one, reorth
      parameter (zero=0.0D0, one=1.0D0, reorth=0.98D0)
      external distdot2
c
c     compute the norm of the input vector
c
      nrm0 = distdot2(n,vec(1,ind),1,vec(1,ind),1)
      ops = ops + n + n
      thr = nrm0 * reorth
...
c
      if (full) then
         do 40 i = ind+1, m
            fct = distdot2(n,vec(1,ind),1,vec(1,i),1)
            hh(i) = fct
            do 20 k = 1, n
...
c
      nrm1 = sqrt(distdot2(n,vec(1,ind),1,vec(1,ind),1))
      ops = ops + n + n
 75   hh(ind) = nrm1
