...
(cd $TRUST_ROOT/externalpackages; md5sum * */* */*/* */*/*/* 2>/dev/null | sort -k 2 | grep -v RELEASE_NOTES | grep -v README_Miniconda | grep -v channeldata | grep -v rss.xml | grep -v repodata.json | grep -v repodata2.json | grep -v index.html | grep -v repodata_from_packages.json | sed "s/ \*/  /") > md5.check
