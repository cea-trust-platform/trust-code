...
do
   ...
   then
      ...
      then
         ...
         # 2>/dev/null sinon warnings "Cannot set native locale, reverting to English" sur Debian 12
         # This avoids patterns like 'for() for() if(...)' on a single line ... 
         sed -i "/^ *\/\//! s@\()\) *\(\(for *(\)\|\(if *(\)\)@\1\n\2@g" $tmp
         LC_ALL="en_US.UTF-8" $TRUST_ROOT/exec/astyle/bin/astyle --keep-one-line-blocks --style=gnu --indent=spaces=2 --indent-cases --align-reference=type --max-instatement-indent=120 $tmp 1>/dev/null 2>/dev/null
         # New version astyle:
         # LC_ALL="en_US.UTF-8" $TRUST_ROOT/exec/astyle/bin/astyle --keep-one-line-blocks --style=gnu --indent=spaces=2 --indent-cases --align-reference=type --max-continuation-indent=120 $tmp 1>/dev/null 2>/dev/null
         rm -f $tmp.orig
         if [ "`diff $file $tmp`" != "" ]
         then
                if [ "$1" != -fix ]
