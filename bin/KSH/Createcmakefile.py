...
if(NOT COMPIL_DYN)
 ...
 IF((  "${ajout}" STREQUAL "" ) OR ($ENV{FORCE_LINK}))
   ...
   file(GLOB_RECURSE LML   FOLLOW_SYMLINKS ${TRUST_TESTS}/[A-Z|a-z]*.lml.gz )
...
if ("${srcs}" STREQUAL "${srcs_atelier}")
  ...
  execute_process(COMMAND ${TRUST_ROOT}/bin/KSH/Cherchefileatelier.py ${listdir} ${LATACOMMON} %% ${srcs} RESULT_VARIABLE toto OUTPUT_VARIABLE srcdeps_new ERROR_VARIABLE error )
...
find_library( libTrio NAMES ${libtrio_name}.a PATHS ${TRUST_ROOT}/lib)
if(EXISTS ${CMAKE_SOURCE_DIR}/cmake.deps)
include(cmake.deps)
endif()
